...
option_list = list(make_option(c("-m", "--memfree"),
                    ...
                    make_option(c("-s", "--spacechunk"),
                                type="integer",
                                default=20,
                                help="The size of the non-time chunking, the same for all non-time dimensions. A higher value will have much faster rechunking speed and better compression, but will be slower when reading a single timeseries [default: %default]"),
                    ...
                    make_option("--suffix",
                                type="character",
                                default="_rechunk",
                                help="String to attach as suffix to the variable name when creating the new variable [default %default]"),
                    make_option("--maxchunk",
                                type="integer",
                                default=0,
                                help="Maximum size of time chunk. Set to 0 to ignore this limit [default %default]"),
                    make_option("--overwrite",
                                action='store_true',
                                help="Overwrite output variable? If not set, renames it if variable already present [default %default]")
)
parser = OptionParser(
    usage = "%prog -v VARIABLE FILENAME",
    option_list=option_list,
    epilogue=paste0("This script rechunks a given netcdf variable from slice to timeseries, creating a new variable in the file. Works around memory limitations by splitting into several jobs. ASSUMES SIGNED VALUES! For feature requests and bug reports, please write to:
    afantini@ictp.it")
)
...
var_already_present = newname %in% names(nc$var)
if (var_already_present) {
    if (overwrite) {
        tellme("The file already has a variable named ", newname, ", WILL OVERWRITE IT!")
    } else {
        renamed = paste0(newname, "_old")
    #     if (renamed %in% names(nc$var)) stop('ERROR! Variable ', renamed, ' already present in the file.')
...
if (var_already_present && overwrite) {
    cs = nc$var[[newname]]$chunksizes
    Tchunk = cs[whichT]
    tellme('Using chunk sizes from already existing variable')
    memreqd = round(prod(cs) * bytes * overhead / 1024**2)
    if (memfree < memreqd + membuffer) stop("ERROR! Not enough memory to carry out the requested operation!")
} else {
    if (memfree < memreqd + membuffer) {
        nj = memreqd %/% (memfree - membuffer) + 1
        if (nj < 1) stop("ERROR! Not enough memory to do anything! Stop being cheap and buy some RAM.")
    ...
    if (maxchunk > 0) {
        if (Tchunk > maxchunk) {
            tellme('Size of calculated chunk (', Tchunk, ') greater than the maximum set chunking size (', maxchunk, '): using the latter')
            Tchunk = maxchunk
        }
    }
    max_short_vectors = 2**31 - 1 #max element size for vactors supported by ncdf4
    varsize = nc$var[[var]]$varsize
    varsize[whichT] = 1
    if (Tchunk * prod(varsize) > max_short_vectors) {
        newTchunk = floor(max_short_vectors / prod(varsize))
        tellme('Long vectors (> 2**31-1 elements) are not supported by ncdf4, so the selected time chunk size (', Tchunk, ') was automatically reduced to ', newTchunk)
        Tchunk = newTchunk
    }
}
cs[whichT] = Tchunk
...
if (exists('renamed')) nc = ncvar_rename(nc, newname, renamed)
...
if (!(var_already_present && overwrite)) {
    ...
    if (length(atts) > 0) {
        invisible(lapply(names(atts), function(n) {
                try( ncatt_put( nc, newname, n, atts[[n]] ) )
        }))
    }
